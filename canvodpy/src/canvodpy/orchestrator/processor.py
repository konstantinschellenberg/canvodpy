...
def preprocess_with_hermite_aux(
    ...
    with tracer_context:
        try:
            ...
            # 1. Read RINEX file (completeness_mode="off" skips epoch
            #    validation scan — we validate at the store level instead)
            rnx = Rnxv3Obs(
                fpath=rnx_file, include_auxiliary=False, completeness_mode="off"
            )
            ds = rnx.to_ds(
