"""Recursive Least Squares with Huber weighting for robust regression."""

from __future__ import annotations

import numpy as np

from canvod.streamstats._types import (
    DEFAULT_HUBER_THRESHOLD,
    DEFAULT_RLS_FORGETTING_FACTOR,
    DEFAULT_RLS_N_FEATURES,
)


class HuberRLS:
    """Streaming robust linear regression via RLS with Huber weighting.

    Same state as RecursiveLeastSquares (beta, P, count, lambda, p) plus
    the Huber threshold c. Observations with large residuals are
    down-weighted using the Huber function: w(r) = 1 if |r| <= c, else c/|r|.

    State layout: [p, lambda, count, huber_c, *beta(p), *P.ravel(p²)]
    Total size: 4 + p + p²
    """

    __slots__ = ("_P", "_beta", "_count", "_huber_c", "_lambda", "_p")

    def __init__(
        self,
        n_features: int = DEFAULT_RLS_N_FEATURES,
        forgetting_factor: float = DEFAULT_RLS_FORGETTING_FACTOR,
        huber_c: float = DEFAULT_HUBER_THRESHOLD,
        initial_covariance: float = 1000.0,
    ) -> None:
        if n_features < 1:
            msg = f"n_features must be >= 1, got {n_features}"
            raise ValueError(msg)
        if not 0.0 < forgetting_factor <= 1.0:
            msg = f"forgetting_factor must be in (0, 1], got {forgetting_factor}"
            raise ValueError(msg)
        if huber_c <= 0.0:
            msg = f"huber_c must be > 0, got {huber_c}"
            raise ValueError(msg)
        self._p = int(n_features)
        self._lambda = float(forgetting_factor)
        self._huber_c = float(huber_c)
        self._beta = np.zeros(self._p, dtype=np.float64)
        self._P = np.eye(self._p, dtype=np.float64) * initial_covariance
        self._count = 0

    @staticmethod
    def _huber_weight(residual: float, c: float) -> float:
        """Huber weight: 1 if |r| <= c, else c/|r|."""
        abs_r = abs(residual)
        if abs_r <= c:
            return 1.0
        return c / abs_r

    def update(self, x: np.ndarray, y: float) -> None:
        """Incorporate a single observation (x, y) with Huber weighting.

        Parameters
        ----------
        x : array-like, shape (p,)
            Feature vector.
        y : float
            Target value.
        """
        x = np.asarray(x, dtype=np.float64).ravel()
        if np.any(np.isnan(x)) or np.isnan(y):
            return
        if len(x) != self._p:
            msg = f"Expected x of length {self._p}, got {len(x)}"
            raise ValueError(msg)

        lam = self._lambda
        Px = self._P @ x
        denom = lam + x @ Px
        K = Px / denom

        error = y - x @ self._beta
        w = self._huber_weight(error, self._huber_c)

        self._beta = self._beta + w * K * error
        self._P = (self._P - np.outer(K, x @ self._P)) / lam
        self._count += 1

    def update_batch(self, X: np.ndarray, y: np.ndarray) -> None:
        """Incorporate a batch of observations sequentially.

        Sequential because Huber weights depend on residuals which change
        after each update.

        Parameters
        ----------
        X : array-like, shape (N, p)
            Feature matrix.
        y : array-like, shape (N,)
            Target vector.
        """
        X = np.asarray(X, dtype=np.float64)
        y = np.asarray(y, dtype=np.float64).ravel()

        if X.ndim == 1:
            X = X.reshape(1, -1)
        if X.shape[1] != self._p:
            msg = f"Expected X with {self._p} columns, got {X.shape[1]}"
            raise ValueError(msg)
        if X.shape[0] != len(y):
            msg = f"X has {X.shape[0]} rows but y has {len(y)} elements"
            raise ValueError(msg)

        for i in range(X.shape[0]):
            self.update(X[i], y[i])

    def merge(self, other: HuberRLS) -> HuberRLS:
        """Merge another HuberRLS accumulator — right-biased (takes higher count).

        Returns self.
        """
        if other._count == 0:
            return self
        if self._count == 0 or other._count > self._count:
            self._p = other._p
            self._lambda = other._lambda
            self._huber_c = other._huber_c
            self._beta = other._beta.copy()
            self._P = other._P.copy()
            self._count = other._count
        return self

    # --- Properties ---

    @property
    def count(self) -> int:
        return self._count

    @property
    def beta(self) -> np.ndarray:
        """Coefficient vector of shape (p,)."""
        return self._beta.copy()

    @property
    def P(self) -> np.ndarray:
        """Covariance matrix of shape (p, p)."""
        return self._P.copy()

    @property
    def n_features(self) -> int:
        return self._p

    @property
    def forgetting_factor(self) -> float:
        return self._lambda

    @property
    def huber_c(self) -> float:
        return self._huber_c

    # --- Serialization ---

    def to_array(self) -> np.ndarray:
        """Serialize to flat array: [p, lambda, count, huber_c, *beta(p), *P.ravel(p²)].

        Shape: (4 + p + p²,)
        """
        p = self._p
        arr = np.empty(4 + p + p * p, dtype=np.float64)
        arr[0] = float(self._p)
        arr[1] = self._lambda
        arr[2] = float(self._count)
        arr[3] = self._huber_c
        arr[4 : 4 + p] = self._beta
        arr[4 + p :] = self._P.ravel()
        return arr

    @classmethod
    def from_array(cls, arr: np.ndarray) -> HuberRLS:
        """Restore from serialized array."""
        data = np.asarray(arr, dtype=np.float64)
        p = int(data[0])
        obj = cls.__new__(cls)
        obj._p = p
        obj._lambda = float(data[1])
        obj._count = int(data[2])
        obj._huber_c = float(data[3])
        obj._beta = data[4 : 4 + p].copy()
        obj._P = data[4 + p : 4 + p + p * p].reshape(p, p).copy()
        return obj
